...
static int spi_nor_fast_read_quad_output(uint start, uint sector_cnt,
					 void *buf);
...
struct spinor_info spi_nor_ids[] = {
    ...
    {0x3725c2, 0x40, 0x00},
    {0x1826c2, 0x40, 0x00},
    {0x1920c2, 0x40, 0x00, QUAD_ENABLE},
    {0x1926c2, 0x40, 0x00},
    {0x1940c8, 0x40, 0x00, QUAD_ENABLE},
    ...
    /* -- PUYA -- */
    {0x186085, 0x02, 0x02, QUAD_ENABLE},
    ...
    /* --XT -- */
    {0x18400b, 0x00, 0x02, QUAD_ENABLE},
...
static int __spinor_pp(uint page_addr, void *buf, uint len)
{
	...
	data_size = sizeof(sdata) / sizeof(sdata[0]);
	memset((void *)sdata, 0xff, data_size);
...
static int __spinor_sector_read(uint start, uint sector_cnt, void *buf)
{
	...
	if (quad_flag == 1) {
		...
		if (ret == -1)
			ret =
			    __spinor_sector_normal_read(start, sector_cnt, buf);
...
static int __spinor_sector_write(uint sector_start, uint sector_cnt, void *buf)
{
	...
	for (i = 0; i < nor_page_cnt; i++) {
		...
		/* 		} */
		ret = __spinor_pp(page_addr + SPINOR_PAGE_SIZE * i,
				  (void *)((uint)buf + SPINOR_PAGE_SIZE * i),
				  SPINOR_PAGE_SIZE);
		if (-1 == ret) {
			return -1;
		}
...
int spinor_init(int stage)
{
	...
	if (script_parser_fetch("boot_spi_board0", "boot_spi_speed_hz",
				&spi_freq, 1) < 0)
...
int spinor_exit(int force)
{
	...
	if (need_exit) {
		...
		{
			if(__spinor_sector_write(spinor_cache_block *
		SPINOR_BLOCK_SECTORS, SPINOR_BLOCK_SECTORS,
		spinor_write_cache))
			{
...
int spinor_read(uint start, uint nblock, void *buffer)
{
	...
	if (spinor_cache_block < 0) {
		...
		if (spinor_cache_block == tmp_block_index) {
			memcpy(buffer,
			       spinor_write_cache +
				   (start % SPINOR_BLOCK_SECTORS) * 512,
			       nblock * 512);
		} else {
			if (__spinor_sector_read(start, nblock, buffer)) {
...
int spinor_write(uint start, uint nblock, void *buffer)
{
	...
	if (spinor_cache_block < 0) {
		...
		if (__spinor_sector_read(
			spinor_cache_block * SPINOR_BLOCK_SECTORS,
			SPINOR_BLOCK_SECTORS, spinor_write_cache)) {
			printf("spinor read cache block fail\n");

			return 0;
		}
		__spinor_erase_block(spinor_cache_block);
		memcpy(spinor_write_cache +
			   (start % SPINOR_BLOCK_SECTORS) * 512,
		       buffer, nblock * 512);
	} else {
		tmp_block_index = start / SPINOR_BLOCK_SECTORS;
...
int spinor_erase(int erase, void *mbr_buffer)
{
	...
	for (i = 0; i < mbr->PartCount; i++) {
		...
		for (start = from; start < (from + nr);
		     start += SPINOR_BLOCK_SECTORS) {
			erase_block = start / SPINOR_BLOCK_SECTORS;
			printf("erasing block index:%x (0x%x)\n", erase_block,
			       (from + nr) / SPINOR_BLOCK_SECTORS);
...
static int __spinor_sprite_sector_write(uint start, uint nsector, void *buffer)
{
	...
	if ((start % SPINOR_BLOCK_SECTORS) ||
	    (nsector < SPINOR_BLOCK_SECTORS)) {
		if (__spinor_sector_read(sector_index * SPINOR_BLOCK_SECTORS,
					 SPINOR_BLOCK_SECTORS,
					 spinor_store_buffer)) {
	...
	memcpy((spinor_store_buffer + ((start % SPINOR_BLOCK_SECTORS) * 512)),
	       buffer, nsector * 512);
	...
	if (__spinor_sector_write(sector_index * SPINOR_BLOCK_SECTORS,
				  SPINOR_BLOCK_SECTORS, spinor_store_buffer)) {
...
int spinor_sprite_write(uint start, uint nblock, void *buffer)
{
	...
	for (start_sector = start + nsector; start_sector < start + nblock;
	     start_sector += SPINOR_BLOCK_SECTORS) {
		if (start + nblock - start_sector < SPINOR_BLOCK_SECTORS)
			sector_once_write = start + nblock - start_sector;

		ret = __spinor_sprite_sector_write(
		    start_sector, sector_once_write,
...
int update_boot0_dram_para(char *buffer)
{
	...
	if (sunxi_verify_checksum((void *)buffer, boot0->boot_head.length,
	...
	memcpy((void *)&boot0->prvt_head.dram_para,
	       (void *)DRAM_PARA_STORE_ADDR, 32 * 4);
	set_boot_dram_update_flag(boot0->prvt_head.dram_para);
	/* regenerate check sum */
	boot0->boot_head.check_sum = sunxi_generate_checksum(
...
static int __spinor_boot_sector_write(uint start, uint nsector, void *buffer)
{
	...
	memcpy((spinor_write_cache + ((start % SPINOR_BLOCK_SECTORS) * 512)),
	       buffer, nsector * 512);
...
int spinor_boot_write(uint start, uint nblock, void *buffer)
{
	...
	for (start_sector = start + nsector; start_sector < start + nblock;
		...
		if (start + nblock - start_sector < SPINOR_BLOCK_SECTORS)
			sector_once_write = start + nblock - start_sector;
		ret = __spinor_boot_sector_write(
		    start_sector, sector_once_write,
		    (void *)((uint)buffer + (start_sector - start) * 512));
		if (ret < 0) {
			printf("spinor sprite write fail\n");
...
int spinor_download_uboot(uint length, void *buffer)
{
	...
	if (uboot_spare_head.boot_data.work_mode == WORK_MODE_BOOT) {
		ret = spinor_boot_write(UBOOT_START_SECTOR_IN_SPINOR,
					length / 512, buffer);
	} else {
		ret = spinor_sprite_write(UBOOT_START_SECTOR_IN_SPINOR,
					  length / 512, buffer);
	}
...
int spinor_datafinish(void)
{
	...
	ret = __spinor_sector_write(0, total_write_bytes / 512,
				    spinor_store_buffer);
	if (ret) {
